...
export class staticsController extends Component {
    ...
    getRandomRichBuff() {
        const buffs = ["股票大涨", "收入翻倍", "高薪工作", "offer", "来财", "一夜暴富","财富自由", "小目标","涨薪", "升职", "暴富","财富", "人民币", "资产", "收入", "财产", "房子", "豪车", "存款", "钞票", "美元", "英镑", "欧元", "日元", "港币", "台币"];
        return buffs[Math.floor(Math.random() * buffs.length)];
    }

    getRandomHealthBuff() {
        const buffs = ["健康", "体力", "精力", "活力", "耐力", "寿命", "大便畅通", "浑身舒畅", "指标正常", "体检通过", "长寿", "长命百岁", "吃得香", "睡得好", "身体棒", "引体向上", "俯卧撑", "仰卧起坐", "长生不老", "清醒", "强壮", "精神", "精神焕发", "精力充沛", "精力旺盛", "精力十足", "精力充沛", "精力充沛", "精力充沛"];
        return buffs[Math.floor(Math.random() * buffs.length)];
    }

    getRandomEmotionBuff() {
        const buffs = ["拒绝内耗", "躺平", "工作轻松", "offer", "平步青云", "颜值爆表", "神操作", "带飞","愉快", "好心情", "快乐", "幸福", "爱情", "情人", "顺利", "放松", "轻松", "美好", "平和", "平静", "一切顺利", "心情愉快", "心情舒畅", "心情美好", "心情美丽"];
        return buffs[Math.floor(Math.random() * buffs.length)];
    }
